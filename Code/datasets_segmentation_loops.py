...
import numpy as np
import mir_eval
from datasets_segmenter import segmenter
...
def beatles_segmentation_loop(beatles_dataset_path,
    ...
    for song_number in range(START_SONG, STOP_SONG):
        ...
        if song_number != 150:
            ...
            if use_ref_beats:
                ref_beats_fname = os.path.join(
                    beatles_dataset_path,
                    'references',
                    'beats_and_downbeats',
                    track_name[:-4] + '.lab')
                estimated_segments = segmenter(audio_filename,
                                               sample_rate=None,
                                               beats_file=ref_beats_fname,
                                               features=features,
                                               criteria=criteria,
...
def beatles_album_track_to_names(num_album, num_track):
    ...
    if num_album == 1:
        ...
        if num_track == 1:
            ...
            raise ValueError('The song number for album {} must lie between 1 '
                             'and {}'.format(album, nb_songs))
    elif num_album == 2:
        ...
        raise ValueError('The album number must lie between 1 and 13.')
    ...
    for song_number in range(len(filenames_list)):
        ...
        try:
            ...
            estimated_boundaries_fname = os.path.join(
                ...
                'segments',
                os.path.splitext(
                    os.path.basename(filenames_list[song_number]))[0]
                + '.lab'
                )
            ...
            try:
                audio_fname = os.path.join(DTL1000_dataset_path,
                                           ...
                                           filenames_list[song_number]
                                           + '.wav')
                estimated_segments = segmenter(audio_fname)[0]
            except FileNotFoundError:
                audio_fname = os.path.join(DTL1000_dataset_path,
                                           'audio',
                                           filenames_list[song_number]
